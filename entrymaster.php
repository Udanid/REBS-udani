...
class Entrymaster extends CI_Controller {
    ...
    function Entrymaster()
    {
		...
		$this->load->model('hm_accresupport_model');
	...
	function printreciepts_duplicate($entrynumber)
    {
        ...
        else
        {
            ...
            if ($entry_type_id > 0) {
			 ...
			 if ($entry_type_id > 0) {
                $this->db->select('ac_entry_items.*,ac_ledgers.name')->join('ac_ledgers','ac_ledgers.id=ac_entry_items.ledger_id')->where('entry_id', $entrynumber);
                //$this->db->join('tchdata','tchdata.TCHID=accclbook.TCHID');
			...
			if($module=='R'){
				...
				//echo  $this->session->userdata('branchid');
					$this->load->view('accounts/entrymaster/printreciepts_duplicate'.$this->session->userdata('branchid'), $data);
					}
			}
			else if($module=='H')
			{
					$data['incomdedata'] = $incomdedata=$this->hm_accresupport_model->get_incomes_by_entryid($entrynumber);
					if($incomdedata){
							$fulldata=NULL;
							$loanbalancedata=NULL;
							$advcount=0;
						$data['branchdata']=$this->hm_accresupport_model->get_branchdata_bycode($incomdedata->branch_code);
					//	//echo  $incomdedata->income_type;
						if($incomdedata->income_type=='Rental Payment' || $incomdedata->income_type=='EP Settlement' )
						{
							$data['fulldata']=$fulldata=$this->hm_accresupport_model->get_eploan_data($incomdedata->temp_code);
							$loanbalancedata=$this->hm_accresupport_model->get_recipt_loan_data($incomdedata->temp_code,$fulldata->loan_type,$fulldata->reschdue_sqn,$fulldata->loan_amount,$fulldata->montly_rental,$incomdedata->income_date);
							$res_code=$fulldata->res_code;
						}
						else
						{
							$advcount=$this->hm_accresupport_model->get_advance_count($incomdedata->temp_code);
							$data['fulldata']=$fulldata=$this->hm_accresupport_model->get_all_reservation_details_bycode($incomdedata->temp_code);	
							$res_code=$fulldata->res_code;
						}
							$cus_code=hm_get_reciept_customer($res_code);
						$data['cusdata']=$cusdata=$this->accresupport_model->get_customer_bycode($cus_code);
						
						$data['loanbalancedata']=$loanbalancedata;
						 $data['advcount']=$advcount;
						$data['resdata']=$this->hm_accresupport_model->get_customer_bycode($incomdedata->cus_code);
						$data['entry_data']=$entry_data = $entry_q->result();
						 $data['ledgers'] = $ledgers->result();
						 $data['mobile'] =  $mobile=$cusdata->mobile;
    ...
    function printreciepts($entrynumber)
    {
        ...
        else
        {
			...
			if($module=='R'){
					...
					$data['entrynumber'] = $entrynumber;
				//echo  $this->session->userdata('branchid');
					$this->load->view('accounts/entrymaster/printreciepts'.$this->session->userdata('branchid'), $data);
					}
			}
			else if($module=='H')
			{
